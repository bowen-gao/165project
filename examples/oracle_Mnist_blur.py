...
def addnoise(x, noise_std):
    ...
    img = np.squeeze(img)
    blurred = ndimage.gaussian_filter(img, sigma=noise_std)
    temp = torch.tensor(blurred)
    x_noise = torch.unsqueeze(torch.unsqueeze(temp, 0), 0)
    return x_noise
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in enumerate(oracle_loader):
            ...
            print("no:", len(step_sizes))
            x_noise = addnoise(x, 3)
            step_sizes, logits = model(x_noise, tol)
            print("add:", len(step_sizes))
            '''
        ...
        with open("oracle/blur_stepnum.txt", 'w') as f:
